...
vec3 GetClouds(in vec3 sky, in vec3 rd)
{
    ...
    vec3 camPos = (uCameraToWorld * vec4(0.0, 0.0, 0.0, 1.0)).xyz;
	float v = (200.0-camPos.y)/rd.y;
	rd.xz *= v;
	rd.xz += camPos.xz;
	rd.xz *= .010;
...
void main()
{
	...
	vec3 col;
	float distance;
	if( !Scene(ro, rd, distance, coords) )
	{
    ...
    imageStore(outputImage, coords, vec4(col, 1.0));
}
